...
import { ChakraProvider, FormLabel, Input, Box, Stack, Text, Flex, Center, Button, Spacer } from '@chakra-ui/react'
import { ChevronRightIcon }           from '@chakra-ui/icons'
import { Html }                       from "@react-three/drei"
import Experience                     from "../../../Experience/Experience"
import { useForm }                    from "react-hook-form";
import { safeTransferFromErc1155Tx }  from "../../../Lib/web3/transactions"
import Contract                       from "../../../Experience/World/Contract";

const SafeTransferFromErc1155: React.FC<{group: string, experience: Experience, props?: any}> = ({ group, experience, props = {} }) => {
  ...
  const onSubmit = async (data) => {
    const tx = await safeTransferFromErc1155Tx(user!.wallet.signer, contract?.interface!, data)
                  ...
                  <Box width={"100%"}>
                    ...
                    <Flex mt="1rem">
                      <Center>
                        ...
                        <Input type="number" min="0" mx="2" p="1" fontSize={"1rem"} borderRadius="md" {...register("id", { required: true })} />
                      </Center>

                      <Spacer />

                      <Center>
                        <Text alignItems={"center"} fontSize={"sm"} >Amount</Text>
                        <Input type="number" min="0" mx="2" p="1" fontSize={"1rem"} borderRadius="md" {...register("amount", { required: true })} />
                      </Center>
                    </Flex>
...
export default SafeTransferFromErc1155 