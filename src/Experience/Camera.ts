...
export default class Camera {
  ...
  setTransitions(): void 
  {
    ...
    */
    ...
    this.transitions.inputsScreen = async (duration: number, target: string) =>
    {
      ...
      const direction = this.experience.world.lootBoxScene?.contracts[target].inputsScreen.getWorldDirection(new Vector3())
      const targetPos = this.experience.world.lootBoxScene?.contracts[target].inputsScreen.position
    ...
    this.transitions.metaScreen = async (duration: number, target: string) =>
    {
      ...
      const direction = this.experience.world.lootBoxScene?.contracts[target].metaScreen.getWorldDirection(new Vector3())
      const targetPos = this.experience.world.lootBoxScene?.contracts[target].metaScreen.position
  ...
  _CalculateIdealOffset() {
    ...
    idealOffset.applyQuaternion(this.experience.world.user?.fox.scene.quaternion);
    idealOffset.add(this.experience.world.user?.fox.scene.position);
    return idealOffset;
  }

  _CalculateIdealLookat() {
    const idealLookat = new THREE.Vector3(0, 0, 20);
    idealLookat.applyQuaternion(this.experience.world.user?.fox.scene.quaternion);
    idealLookat.add(this.experience.world.user?.fox.scene.position);
    return idealLookat;
  }

  _CalculateIdealLookatPointing() {
    const idealLookat = new THREE.Vector3(0, 2, 0);
    idealLookat.applyQuaternion(this.user?.fox.scene.quaternion);
    idealLookat.add(this.user?.fox.scene.position);
    return idealLookat;
  }
